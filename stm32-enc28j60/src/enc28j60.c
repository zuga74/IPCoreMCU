...
void ENC28J60_SendByteSnd(uint8_t tx)
{
	if (hspi == NULL) return;
	HAL_SPI_Transmit(hspi, &tx, 1, 0xffffffff);
}

unsigned char ENC28J60_SendRcvByte(uint8_t tx)
{
	if (hspi == NULL)
		return 0;
...
uint8_t enc28j60ReadOp(uint8_t op, uint8_t address)
{
        ...
        enableChip;
        // issue read command
        ENC28J60_SendByteSnd(op | (address & ADDR_MASK));
        temp = ENC28J60_SendRcvByte(0xFF);
        if (address & 0x80)
            temp = ENC28J60_SendRcvByte(0xFF);
...
{
    enableChip;
    ENC28J60_SendByteSnd(ENC28J60_READ_BUF_MEM);
    while (len--) {
        *data++ = ENC28J60_SendRcvByte(0x00);
    }
    disableChip;
...
{
    enableChip;
    ENC28J60_SendByteSnd(ENC28J60_WRITE_BUF_MEM);
    while (len--)
        ENC28J60_SendByteSnd(*data++);
